...
//SDK相关的
static rtw_network_info_t wifi = {0};   //这个是从at指令那边copy来的
static rtw_softap_info_t ap = {0};  //这个是从at指令那边copy来的
...
aiio_err_t aiio_wifi_init(void)
{
    ...
    wifi_reg_event_handler(WIFI_EVENT_JOIN_STATUS,aiio_wifi_join_status_cb);    //20240903同步SDK更新wifi回调事件
...
aiio_err_t aiio_wifi_set_mode(aiio_wifi_mode_t mode)
{
    ...
    enum rtw_mode_type rtl_wifiMode=RTW_MODE_NONE;
    ...
    //wifi_off();   //WAIT_TODO 原始SDK好像没有实现
    vTaskDelay(20);
    if (wifi_on(rtl_wifiMode) != RTW_SUCCESS)
    {
...
aiio_err_t aiio_wifi_scan(void *data, aiio_scan_complete_cb_t cb, bool filter)
{
	...
	if (wifi_scan_networks(&scan_param, 0) != RTW_SUCCESS) {    //wifi_scan_networks(aiio_wifi_scan_result_cb, NULL)
...
aiio_err_t aiio_wifi_scan_adv(void *data, aiio_scan_complete_cb_t cb, bool filter, aiio_wifi_scan_params_t *scan_params)
{
    aiio_err_t ret = AIIO_ERROR;
    enum rtw_join_status_type join_status = RTW_JOINSTATUS_UNKNOWN;
    struct _rtw_scan_param_t rtl_scan_param = {RTW_SCAN_NOUSE, 0, 0, 0, {0}, 0, 0, 0, 0};
    ...
    s_scan_ap_num = 0;
    s_wifi_scan_done = false;
    if (wifi_scan_networks(&rtl_scan_param, 0) != RTW_SUCCESS) {
		aiio_log_e("wifi_scan_networks ERROR");
		ret = AIIO_ERROR;
        goto __err_free;
	}
...
aiio_err_t aiio_wifi_sta_mac_get(uint8_t *mac)
{
	...
	p_wifi_setting = (struct _rtw_wifi_setting_t *)aiio_os_malloc(sizeof(struct _rtw_wifi_setting_t));
...
aiio_err_t aiio_wifi_set_country_code(char *country_code){
    aiio_log_e("WAIT_TOD func wait to define");
    return AIIO_ERROR;
}

aiio_err_t aiio_wifi_channel_get(int32_t *channel)
{
    if (wifi_get_channel(STA_WLAN_INDEX,channel) != RTW_SUCCESS)
    {
        return AIIO_ERROR;
    }
...
{
     if (wifi_set_channel(STA_WLAN_INDEX,channel) != RTW_SUCCESS)
    {
        return AIIO_ERROR;
    }
...
aiio_err_t aiio_wifi_sta_connect_ind_stat_get(aiio_wifi_sta_connect_ind_stat_info_t *wifi_ind_stat)
{
	...
	for (int i = 0; i < NET_IF_NUM; i++) {
		if (wifi_is_running(i)) {
            ...
            memcpy(wifi_ind_stat->bssid, tmp_mac, 6);
...
static rtw_result_t app_scan_result_handler(unsigned int scanned_AP_num, void *user_data)
{
	...
	scan_buf = (char *)aiio_os_malloc(scanned_AP_num * sizeof(rtw_scan_result_t));
	if (scan_buf == NULL) {
